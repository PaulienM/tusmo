...
export class Guess {
    ...
    constructor(solution: string, guess: string) {
        ...
        this.feedback = guessArray.map((value) => ({letter: value, status: GuessLetterStatusEnum.INCORRECT}));
        ...
        const solutionWithoutCorrect = solution.split('').filter((_, index) => this.feedback[index].status !== GuessLetterStatusEnum.CORRECT);
        guessArray.forEach((value, index) => {
